...
import { RealtimeNoticeTracker, noticeText, noticesFromSnapshot, snapshotKeys } from './submissionAlerts'
...
import { activeGameStatus, roomAttention } from './roomFlow'
...
export default function App(){
  ...
  const [notices,setNotices]=useState<string[]>([]),[applyUpdate,setApplyUpdate]=useState<null|(()=>void)>(null)
  ...
  const resultsRound=room?completedResultsRound(room):0
  ...
  const refresh=useCallback(async(s:LocalSession)=>{
    ...
    if(snapshotRef.current){for(const event of noticesFromSnapshot(snapshotRef.current,next,s.playerId)){if(alertTracker.current.accept(event))setNotices(q=>[...q,noticeText(event)])}}
  ...
  const home=()=>{setScreen('home');setSession(null);snapshotRef.current=null;setRoom(null);setPlayers([]);setAttempts([]);setResult(null);setRoomView('game');setError('');loadActiveGames()}
  ...
  if(screen==='lobby'&&room&&session&&roomView==='results')return <main className="panel room-results"><RoomTabs view={roomView} setView={setRoomView} hasResults={currentResults.length>0}/><div className="room-status"><strong>{attention?.title}</strong><span>{attention?.detail}</span></div><span className="eyebrow">Round {resultsRound} results</span><h2>Round standings</h2><RoundResults rows={currentResults} myId={session.playerId}/>{room.last_completed_round>=3&&<button className="secondary-wide" onClick={()=>setRoomView('full')}>View Full Match Results</button>}{room.status==='drawing'&&<button className="primary dock" onClick={()=>{setRoomView('game');refresh(session)}}>Draw Round {room.current_round} <b>→</b></button>}{session.isHost&&<button className="end-game" onClick={endGame}>End Game</button>}{noticeControl}{updateControl}</main>
