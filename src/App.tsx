...
import { RealtimeNotice, RealtimeNoticeTracker, noticeText, noticesFromSnapshot, snapshotKeys } from './submissionAlerts'
import { isDrawableLifecycle, MultiplayerLifecycle, resolveMultiplayerState } from './roundLifecycle'
import { circleResultPreview, completedResultsRound, rankedRound, roundHistory, sessionRows } from './roomResults'
import { identityForRoom, sharedRoomCode, sharedRoomUrl } from './sharedRoom'
import { activeGameStatus, activeGameTone, resultPrimaryAction, roomAttention } from './roomFlow'
...
type RoomView='game'|'results'|'history'|'session'|'full'
type ActiveGame = { session:LocalSession; room:Room; players:Player[]; attempts:Attempt[]; participants:RoundParticipant[] }
...
export default function App(){
  ...
  const [notices,setNotices]=useState<RealtimeNotice[]>([]),[applyUpdate,setApplyUpdate]=useState<null|(()=>void)>(null)
  ...
  const alertTracker=useRef(new RealtimeNoticeTracker()), snapshotRef=useRef<{room:Room;players:Player[];attempts:Attempt[]}|null>(null)
  const activeSnapshots=useRef(new Map<string,{room:Room;players:Player[];attempts:Attempt[]}>()),activeTrackers=useRef(new Map<string,RealtimeNoticeTracker>())
  ...
  const resultsRound=revealRound||(room?completedResultsRound(room):0)
  ...
  const resultAction=room?resultPrimaryAction(resultsRound,room.current_round,room.match_length,room.is_extended):null
  const attention=room&&session?roomAttention(room,players,attempts,session.playerId,participants):null
  ...
  const refresh=useCallback(async(s:LocalSession)=>{
    ...
    const sequence=++refreshSequence.current,db=supabase;if(screenRef.current!=='draw')setLifecycle('loading')
    const{data:r,error:roomError}=await db.from('rooms').select('*').eq('id',s.roomId).maybeSingle()
    if(sequence!==refreshSequence.current)return false
    if(roomError){setError('We could not refresh this game. Check your connection and try again.');return false}
    if(!r){removeSession(s.roomId);setError('Game could not be found.');setScreen('home');return}
    const [playerRows,attemptRows,participantRows,mineRow]=await Promise.all([
      ...
      r.current_round>0?db.from('attempts').select('*').eq('room_id',s.roomId).eq('player_id',s.playerId).eq('round_number',r.current_round).maybeSingle():Promise.resolve({data:null,error:null}),
    ])
    if(sequence!==refreshSequence.current)return false
    const fetchError=playerRows.error||attemptRows.error||participantRows.error||mineRow.error
    if(fetchError){setError('We could not refresh this game. Check your connection and try again.');return false}
    const p=playerRows.data,a=attemptRows.data,rp=participantRows.data,mine=mineRow.data
    ...
    const next={room:r as Room,players:(p||[]) as Player[],attempts:(a||[]) as Attempt[]}
    if(snapshotRef.current){for(const event of noticesFromSnapshot(snapshotRef.current,next,s.playerId)){if(alertTracker.current.accept(event))setNotices(q=>[...q,event])}}
    else alertTracker.current.seed(snapshotKeys(next.room,next.players,next.attempts))
    ...
    else if(screenRef.current!=='draw'){
      const route=smartRoomView(r,s.playerId,next.attempts);setScreen('lobby')
      if(route.view==='results'){setRevealRound(route.round!);setRoomView('results')}
      else if(route.view==='full')setRoomView('full')
      else setRoomView('game')
    }
    return true
  },[])
  const loadActiveGames=useCallback(async()=>{
    ...
    const games=(await Promise.all(saved.map(async s=>{const [{data:r},{data:p},{data:a},{data:rp}]=await Promise.all([db.from('rooms').select('*').eq('id',s.roomId).neq('status','closed').maybeSingle(),db.from('players').select('*').eq('room_id',s.roomId).eq('is_active',true).order('joined_at'),db.from('attempts').select('*').eq('room_id',s.roomId),db.from('round_participants').select('*').eq('room_id',s.roomId)]);return r?{session:s,room:r,players:p||[],attempts:a||[],participants:rp||[]}:null}))).filter(Boolean) as ActiveGame[]
    for(const game of games){const next={room:game.room,players:game.players,attempts:game.attempts},previous=activeSnapshots.current.get(game.room.id);let tracker=activeTrackers.current.get(game.room.id);if(!tracker){tracker=new RealtimeNoticeTracker();tracker.seed(snapshotKeys(next.room,next.players,next.attempts));activeTrackers.current.set(game.room.id,tracker)}else if(previous){for(const event of noticesFromSnapshot(previous,next,game.session.playerId))if(tracker.accept(event))setNotices(q=>[...q,event])}activeSnapshots.current.set(game.room.id,next)}
  ...
  useEffect(()=>{if(!supabase||screen!=='home'||!activeGames.length)return;const db=supabase,channels=activeGames.map(g=>db.channel(`active:${g.room.id}`).on('postgres_changes',{event:'*',schema:'public',table:'players',filter:`room_id=eq.${g.room.id}`},loadActiveGames).on('postgres_changes',{event:'*',schema:'public',table:'attempts',filter:`room_id=eq.${g.room.id}`},loadActiveGames).on('postgres_changes',{event:'*',schema:'public',table:'rooms',filter:`id=eq.${g.room.id}`},loadActiveGames).subscribe());return()=>{channels.forEach(ch=>db.removeChannel(ch))}},[screen,activeGames.map(g=>g.room.id).join(','),loadActiveGames])
  useEffect(()=>{if(roomView==='game'&&session)refresh(session)},[roomView,session,refresh])
  useEffect(()=>{if(roomView==='results'&&room&&session&&resultsRound)markRoundViewed(room.id,session.playerId,resultsRound)},[roomView,room?.id,session?.playerId,resultsRound])
  useEffect(()=>{
  ...
  useEffect(()=>{if(!notices.length)return;const timer=setTimeout(()=>setNotices(q=>q.slice(1)),5200);return()=>clearTimeout(timer)},[notices])
  useEffect(()=>{
    ...
    const ch=db.channel(`room:${session.roomId}`)
      ...
      .on('postgres_changes',{event:'UPDATE',schema:'public',table:'rooms',filter:`id=eq.${session.roomId}`},()=>refresh(session)).subscribe(status=>{if(status==='SUBSCRIBED')refresh(session)})
  ...
  const home=()=>{setScreen('home');setSession(null);snapshotRef.current=null;setRoom(null);setPlayers([]);setAttempts([]);setResult(null);setRevealRound(0);setRoomView('game');setError('');loadActiveGames()}
  ...
  async function submit(points:Point[],score:ScoreResult){setResult({points,score});if(!score.valid)return;if(solo){setScreen('result');localStorage.setItem(BEST_KEY,String(Math.max(score.score,Number(localStorage.getItem(BEST_KEY)||0))));return}if(!room||!session||!isDrawableLifecycle(lifecycle))return;setLifecycle('loading');const authoritativeRound=room.current_round;const{data:persisted,error:e}=await supabase!.from('attempts').insert({room_id:room.id,player_id:session.playerId,round_number:authoritativeRound,score:score.score,rating:ratingFor(score.score),points:simplify(points),radial_error:score.radialError,closure_error:score.closureRatio,smoothness_score:score.smoothness,angular_coverage:score.angularCoverage}).select().single();if(e){setError(/unique|duplicate/i.test(e.message)?'You already submitted this round.':'Your circle could not be submitted.');setScreen('lobby');await refresh(session);return}if(persisted)setAttempts(old=>[...old.filter(a=>a.id!==persisted.id),persisted]);await supabase!.rpc('complete_circle_round',{p_room_id:room.id,p_round_number:authoritativeRound});setResult(null);setScreen('lobby');await refresh(session)}
  ...
  const notice=notices[0],noticeCopy=notice?noticeText(notice):''
  const noticeControl=notice?createPortal(<div className={`game-notice ${notice.kind}`} role="status" aria-live="polite"><strong>{noticeCopy.split(' — ')[0].toUpperCase()}</strong>{noticeCopy.includes(' — ')&&<span>{noticeCopy.split(' — ')[1]}</span>}{notice.kind==='round_complete'&&<button onClick={()=>{setRevealRound(notice.round);setRoomView('results');setScreen('lobby');setNotices(q=>q.slice(1))}}>View Results</button>}{notice.kind==='round_ready'&&<button onClick={()=>{setRoomView('game');setScreen('lobby');setNotices(q=>q.slice(1))}}>Draw Now</button>}{notices.length>1&&<small>{notices.length-1} more queued</small>}</div>,document.body):null

  if(screen==='draw'&&!solo&&!isDrawableLifecycle(lifecycle))return <LoadingGame retry={()=>session?refresh(session):false} notice={noticeControl}/>

  if(screen==='home')return <main className="home"><header><Mark/><span className="eyebrow">A very serious game</span><h1>CIRCLE<span>.</span></h1><p>Draw one. Find out how round you really are.</p></header>{activeGames.length>0&&<section className="active-games"><span className="eyebrow">Active games</span>{activeGames.map(g=>{const unseen=unseenCompletedRound(g.room,g.session.playerId,g.attempts),tone=unseen?'results':activeGameTone(g.room,g.players,g.attempts,g.session.playerId,g.participants);const status=activeGameStatus(g.room,g.players,g.attempts,g.session.playerId,g.participants);return <div className={`active-game ${tone}`} key={g.room.id}><span className="active-game-copy"><span className="active-game-heading"><strong>{g.room.room_code}</strong><b className="attention-label">{unseen?'RESULTS READY':tone==='turn'?'YOUR TURN':'WAITING'}</b></span><small>{status}</small></span><button className="open-game" onClick={()=>openGame(g)}>Open</button><button className="remove" aria-label={`Remove ${g.room.room_code}`} onClick={()=>forget(g.room.id)}>×</button></div>})}</section>}<div className="home-actions"><button className="primary" onClick={()=>{setMode('create');setScreen('form')}}>Create game <b>↗</b></button><button onClick={()=>{setMode('join');setScreen('form')}}>Join game <b>→</b></button><button className="ghost" onClick={()=>{setMode('solo');setScreen('draw')}}>Solo practice <b>◎</b></button></div><footer>No sign-up. No geometry degree. · v{__APP_VERSION__}</footer>{noticeControl}{updateControl}</main>
  ...
  if(screen==='lobby'&&room&&session&&roomView==='full')return <main className="panel full-match"><RoomTabs view={roomView} setView={setRoomView} hasResults/><span className="eyebrow">All completed rounds</span><h2>Full Match Results</h2><FullMatchResults attempts={attempts} players={players} through={room.last_completed_round} myId={session.playerId}/>{noticeControl}{updateControl}</main>
  if(screen==='lobby'&&room&&session&&isFinalMatch(room)&&roomView!=='results')return <main className="panel final-match"><RoomTabs view="results" setView={v=>{if(v!=='results')setRoomView(v)}} hasResults onHome={home}/><div className="room-status"><strong>{attention?.title}</strong><span>{attention?.detail}</span></div><span className="eyebrow">Game winner</span><h2>{matchStandings[0]?.tied?'It’s a tie':matchStandings[0]?.player.display_name||'Final results'}</h2><FinalMatch rows={matchStandings}/><button className="secondary-wide" onClick={()=>setRoomView('full')}>View Full Match Results</button>{session.isHost&&<button className="primary dock" onClick={keepPlaying}>Keep Playing <b>→</b></button>}{!session.isHost&&<p className="waiting">Game complete</p>}{noticeControl}{updateControl}</main>
  if(screen==='lobby'&&room&&session&&roomView==='results')return <main className="panel room-results"><RoomTabs view={roomView} setView={setRoomView} hasResults={currentResults.length>0}/><div className="room-status"><strong>{attention?.title}</strong><span>{attention?.detail}</span></div><span className="eyebrow">Round {resultsRound} results</span><h2>{currentResults.length>1&&currentResults[0].score===currentResults[1].score?'Round tied':currentResults[0]?`${currentResults[0].playerName} wins`:'Round results'}</h2><RoundResults rows={currentResults} myId={session.playerId}/>{resultAction?.kind==='full'?<button className="primary dock" onClick={()=>setRoomView('full')}>{resultAction.label} <b>→</b></button>:room.current_round>resultsRound?<button className="primary dock" onClick={()=>{setRevealRound(0);setRoomView('game');refresh(session)}}>{resultAction?.label} <b>→</b></button>:null}{session.isHost&&<button className="end-game" onClick={endGame}>End Game</button>}{noticeControl}{updateControl}</main>
  if(screen==='lobby'&&room&&session&&roomView==='history')return <main className="panel"><RoomTabs view={roomView} setView={setRoomView} hasResults={currentResults.length>0}/><div className="room-status"><strong>{attention?.title}</strong><span>{attention?.detail}</span></div><span className="eyebrow">Room {room.room_code}</span><h2>Round history</h2><History history={history}/>{room.last_completed_round>=3&&<button className="secondary-wide" onClick={()=>setRoomView('full')}>View Full Match Results</button>}{noticeControl}{updateControl}</main>
  if(screen==='lobby'&&room&&session&&roomView==='session')return <main className="panel"><RoomTabs view={roomView} setView={setRoomView} hasResults={currentResults.length>0}/><div className="room-status"><strong>{attention?.title}</strong><span>{attention?.detail}</span></div><span className="eyebrow">This room</span><h2>Session leaderboard</h2><SessionLeaderboard rows={sessionStats}/>{noticeControl}{updateControl}</main>
  if(screen==='lobby'&&room&&session)return <main className="panel lobby"><RoomTabs view={roomView} setView={setRoomView} hasResults={currentResults.length>0}/><div className="top"><button className="back" onClick={home}>← Home</button><button className="text-btn" onClick={()=>setRoomView(room.status==='results'?'results':'session')}>{room.status==='results'?'View results':'Leaderboard'}</button></div><span className="eyebrow">Room code</span><h2 className="room-code">{room.room_code}</h2><button className="share" onClick={share}>{copied?'Copied!':'Share game'} <b>↗</b></button>{session.isHost&&<div className="reveal"><span>Reveal scores</span><button aria-pressed={room.reveal_mode==='all_submitted'} onClick={async()=>{const v=room.reveal_mode==='all_submitted'?'immediate':'all_submitted';setRoom({...room,reveal_mode:v});await supabase!.from('rooms').update({reveal_mode:v}).eq('id',room.id)}}><i/>{room.reveal_mode==='all_submitted'?'When everyone finishes':'As players finish'}</button></div>}<div className="room-status"><strong>{attention?.title}</strong><span>{attention?.detail}</span></div><section><div className="section-title"><h3>Players</h3><span>{players.length} / {room.expected_player_count} joined · {submittedIds.size} submitted</span></div><ul className="players">{players.map(p=>{const submitted=submittedIds.has(p.id),included=activeParticipantIds.has(p.id);return <li key={p.id}><span className="avatar">{p.display_name[0].toUpperCase()}</span><strong>{p.display_name}{p.id===session.playerId?' (you)':''}</strong>{p.is_host&&<em>Host</em>}<span className={submitted?'submitted':'drawing'}>{room.status==='lobby'?'Ready':!included?'Next round':submitted?'Submitted ✓':'Drawing…'}</span>{session.isHost&&!p.is_host&&<button className="kick" aria-label={`Remove ${p.display_name}`} onClick={()=>removePlayer(p.id)}>Remove</button>}</li>})}</ul></section>{room.status==='drawing'&&<><p className="waiting">{submittedIds.has(session.playerId)?<>Submitted ✓<br/>{players.length<room.expected_player_count?'Waiting for players':`Waiting for ${players.length-submittedIds.size} attempt(s)`}<br/>{players.length} / {room.expected_player_count} joined</>:lifecycle==='next_round_ready'?`Round ${room.current_round} ready — your turn`:'Your turn'}</p>{!submittedIds.has(session.playerId)&&<button className="primary dock" onClick={()=>setScreen('draw')}>Draw Round {room.current_round} <b>→</b></button>}{session.isHost&&room.expected_player_count>players.length&&<button className="text-btn" onClick={changePlayerCount}>Change player count</button>}</>}{error&&<p className="error" role="alert">{error}</p>}{noticeControl}{updateControl}</main>
  if(screen==='draw')return <main className="game"><div className="game-head"><button className="back" onClick={home}>← Home</button><span>{solo?'Solo practice':`Round ${room?.current_round}`}</span><span className="dot">● LIVE</span></div><h2>Draw one circle.</h2><p>One stroke. No do-overs.</p><DrawPad onDone={submit}/>{result&&!result.score.valid&&<div className="toast" role="alert">{result.score.reason}<button onClick={()=>setResult(null)}>Try again</button></div>}<div className="tip"><b>TIP</b> Slow and steady beats fast and wobbly.</div>{noticeControl}</main>
...
function RoundResults({rows,myId}:{rows:ReturnType<typeof rankedRound>;myId:string}){return <ol className="round-results">{rows.map(row=><li key={row.id} className={`${row.winner?'winner ':''}${row.player_id===myId?'mine':''}`}><div className="result-circle"><DrawPad preview={circleResultPreview(row)}/></div><div className="result-rank"><b>#{row.rank}</b>{row.winner&&<i>Winner</i>}</div><span><strong>{row.playerName}{row.player_id===myId?' (you)':''}</strong><small>{row.rating}</small></span><em>{row.score.toFixed(1)}<small>score</small></em></li>)}</ol>}
function FullMatchResults({attempts,players,through,myId}:{attempts:Attempt[];players:Player[];through:number;myId:string}){const rows=finalMatchStandings(attempts.filter(a=>a.round_number<=through),players);const rounds=roundHistory(attempts,players,through+1);return <><span className="eyebrow">Overall match summary</span><h3>{rows[0]?.tied?'Overall tie':rows[0]?`${rows[0].player.display_name} leads`:'No completed rounds'}</h3><FinalMatch rows={rows}/>{rounds.map(round=><section className="full-round" key={round.roundNumber}><h3>Round {round.roundNumber}</h3><RoundResults rows={round.results} myId={myId}/></section>)}</>}
