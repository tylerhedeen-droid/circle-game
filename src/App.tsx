...
import { RealtimeNotice, RealtimeNoticeTracker, noticeText } from './submissionAlerts'
...
export default function App(){
  ...
  const alertTracker=useRef(new RealtimeNoticeTracker()), seedComplete=useRef(false), playersRef=useRef(players)
  ...
  useEffect(()=>{
    ...
    const db=supabase;seedComplete.current=false;alertTracker.current=new RealtimeNoticeTracker()
    Promise.all([db.from('players').select('id').eq('room_id',session.roomId),db.from('attempts').select('id').eq('room_id',session.roomId),db.from('rooms').select('current_round,last_completed_round').eq('id',session.roomId).single()]).then(([{data:p},{data:a},{data:r}])=>{alertTracker.current.seed([...(p||[]).map(x=>`player:${x.id}`),...(a||[]).map(x=>`attempt:${x.id}`),...(r?.last_completed_round?[`round_complete:${r.last_completed_round}`]:[]),...(r&&r.current_round>1?[`round_ready:${r.current_round}`]:[])]);seedComplete.current=true})
    const announce=(event:RealtimeNotice,strong=false)=>{if(!seedComplete.current||!alertTracker.current.accept(event))return;setNotice(noticeText(event));navigator.vibrate?.(strong?[70,45,70]:35);setTimeout(()=>setNotice(''),strong?3500:2600)}
    const ch=db.channel(`room:${session.roomId}`)
      .on('postgres_changes',{event:'INSERT',schema:'public',table:'players',filter:`room_id=eq.${session.roomId}`},payload=>{const p=payload.new as Player;if(p.id!==session.playerId)announce({kind:'player_joined',playerId:p.id,playerName:p.display_name})})
      ...
      .on('postgres_changes',{event:'INSERT',schema:'public',table:'attempts',filter:`room_id=eq.${session.roomId}`},payload=>{const a=payload.new as Attempt;if(a.player_id===session.playerId)return;const who=playersRef.current.find(p=>p.id===a.player_id)?.display_name||'A player';announce({kind:'round_submitted',attemptId:a.id,playerName:who,round:a.round_number})})
      .on('postgres_changes',{event:'*',schema:'public',table:'attempts',filter:`room_id=eq.${session.roomId}`},()=>refresh(session))
      .on('postgres_changes',{event:'UPDATE',schema:'public',table:'rooms',filter:`id=eq.${session.roomId}`},payload=>{const after=payload.new as Room;if(after.last_completed_round)announce({kind:'round_complete',round:after.last_completed_round},true);if(after.status==='drawing'&&after.current_round>1)setTimeout(()=>announce({kind:'round_ready',round:after.current_round},true),1200);refresh(session)}).subscribe()
