...
import { completedRoundResults, mayRevealCurrent, rankedRound, roundHistory, sessionRows } from './roomResults'
...
type ActiveGame = { session:LocalSession; room:Room; playerCount:number; submitted:boolean; submittedCount:number }
...
function DrawPad({onDone,preview,compact=false}:{onDone?:(points:Point[],r:ScoreResult)=>void;preview?:{points:Point[];score:ScoreResult};compact?:boolean}){
  ...
  return <canvas ref={canvas} className={`draw-pad${compact?' compact':''}`} aria-label={preview?'Submitted circle preview':'Circle drawing area'}
...
export default function App(){
  ...
  const alertTracker=useRef(new SubmissionAlertTracker()), seedComplete=useRef(false), playersRef=useRef(players),refreshSequence=useRef(0),screenRef=useRef(screen)
  playersRef.current=players;screenRef.current=screen
  ...
  const currentResults=room?completedRoundResults(room,attempts,players):[]
  ...
  const refresh=useCallback(async(s:LocalSession)=>{
    if(!supabase)return
    const request=++refreshSequence.current,db=supabase
    ...
    else if(isDrawableLifecycle(resolved)){if(screenRef.current!=='draw')setScreen('lobby');setRoomView('game')}
  ...
  const loadActiveGames=useCallback(async()=>{
    ...
    const games=(await Promise.all(saved.map(async s=>{const [{data:r},{count},{data:a}]=await Promise.all([db.from('rooms').select('*').eq('id',s.roomId).neq('status','closed').maybeSingle(),db.from('players').select('*',{count:'exact',head:true}).eq('room_id',s.roomId).eq('is_active',true),db.from('attempts').select('round_number,player_id').eq('room_id',s.roomId)]);const current=(a||[]).filter(x=>x.round_number===r?.current_round);return r?{session:s,room:r,playerCount:count||0,submitted:current.some(x=>x.player_id===s.playerId),submittedCount:current.length}:null}))).filter(Boolean) as ActiveGame[]
  ...
  useEffect(()=>{
    ...
    Promise.all([db.from('attempts').select('id').eq('room_id',session.roomId),db.from('players').select('id').eq('room_id',session.roomId),db.from('rooms').select('last_completed_round').eq('id',session.roomId).single()]).then(([a,p,r])=>{alertTracker.current.seed([...(a.data||[]).map(x=>`attempt:${x.id}`),...(p.data||[]).map(x=>`player:${x.id}`),...Array.from({length:r.data?.last_completed_round||0},(_,i)=>`round:${i+1}`)]);seedComplete.current=true})
    ...
    const ch=db.channel(`room:${session.roomId}`)
      .on('postgres_changes',{event:'INSERT',schema:'public',table:'players',filter:`room_id=eq.${session.roomId}`},payload=>{const p=payload.new as Player;if(seedComplete.current&&p.id!==session.playerId&&alertTracker.current.accept(`player:${p.id}`))announce(playerJoinedNotice(p.display_name));refresh(session)})
      .on('postgres_changes',{event:'UPDATE',schema:'public',table:'players',filter:`room_id=eq.${session.roomId}`},()=>refresh(session))
      .on('postgres_changes',{event:'*',schema:'public',table:'round_participants',filter:`room_id=eq.${session.roomId}`},()=>refresh(session))
      .on('postgres_changes',{event:'INSERT',schema:'public',table:'attempts',filter:`room_id=eq.${session.roomId}`},payload=>{const a=payload.new as Attempt;if(!seedComplete.current||a.player_id===session.playerId||!alertTracker.current.accept(`attempt:${a.id}`))return;const who=playersRef.current.find(p=>p.id===a.player_id)?.display_name||'A player';announce(playerSubmittedNotice(who,a.round_number))})
      .on('postgres_changes',{event:'*',schema:'public',table:'attempts',filter:`room_id=eq.${session.roomId}`},()=>refresh(session))
      .on('postgres_changes',{event:'UPDATE',schema:'public',table:'rooms',filter:`id=eq.${session.roomId}`},payload=>{const after=payload.new as Room;if(seedComplete.current&&after.last_completed_round>0&&alertTracker.current.accept(`round:${after.last_completed_round}`)){announce(roundCompleteNotice(after.last_completed_round),true);if(after.status==='drawing')setTimeout(()=>announce(roundReadyNotice(after.current_round),true),1200)}refresh(session)}).subscribe()
  ...
  if(screen==='lobby'&&room&&session&&isFinalMatch(room))return <main className="panel final-match"><RoomTabs view="results" setView={v=>{if(v!=='results')setRoomView(v)}} hasResults onHome={home}/><span className="eyebrow">Game winner</span><h2>{matchStandings[0]?.tied?'It’s a tie':matchStandings[0]?.player.display_name||'Final results'}</h2><FinalMatch rows={matchStandings}/><h3>Round {resultsRound} circles</h3><RoundResults rows={currentResults} myId={session.playerId}/><History history={roundHistory(attempts,players,room.current_round+1)}/>{session.isHost&&<button className="primary dock" onClick={keepPlaying}>Keep Playing <b>→</b></button>}{!session.isHost&&<p className="waiting">Game complete</p>}{updateControl}</main>
...
function RoundResults({rows,myId}:{rows:ReturnType<typeof rankedRound>;myId:string}){return <ol className="round-results comparison-grid">{rows.map(row=>{const fit=scoreStroke(row.points);return <li key={row.id} className={`${row.winner?'winner ':''}${row.player_id===myId?'mine':''}`}><div className="result-card-head"><b>{row.rank}</b><span><strong>{row.playerName}{row.player_id===myId?' (you)':''}</strong><small>{row.rating}</small></span><em>{row.score.toFixed(1)}</em></div><DrawPad compact preview={{points:row.points,score:fit}}/>{row.winner&&<i>Winner</i>}</li>})}</ol>}
