...
export default function App(){
  ...
  const [players,setPlayers]=useState<Player[]>([]),[attempts,setAttempts]=useState<Attempt[]>([]),[activeGames,setActiveGames]=useState<ActiveGame[]>([])
  ...
  const refresh=useCallback(async(s:LocalSession)=>{
    if(!supabase)return
    const [{data:r},{data:p},{data:a}]=await Promise.all([supabase.from('rooms').select('*').eq('id',s.roomId).single(),supabase.from('players').select('*').eq('room_id',s.roomId).order('joined_at'),supabase.from('attempts').select('*').eq('room_id',s.roomId)])
    if(r)setRoom(r);if(p)setPlayers(p);if(a)setAttempts(a)
    const current=(a||[]).filter(x=>x.round_number===r?.current_round)
    if(s.isHost&&r?.status==='drawing'&&p?.length&&current.length>=p.length){await supabase.from('rooms').update({status:'results'}).eq('id',s.roomId);setRoom({...r,status:'results'});setScreen('result')}
    else if(r?.status==='drawing'&&!current.some(x=>x.player_id===s.playerId))setScreen('draw')
    else if(r?.status==='results')setScreen('result')
  },[])
  const loadActiveGames=useCallback(async()=>{
    if(!supabase)return
    const db=supabase
    const saved=Object.values(readSessions().sessions)
    const games=(await Promise.all(saved.map(async s=>{const [{data:r},{count}]=await Promise.all([db.from('rooms').select('*').eq('id',s.roomId).neq('status','closed').maybeSingle(),db.from('players').select('*',{count:'exact',head:true}).eq('room_id',s.roomId)]);return r?{session:s,room:r,playerCount:count||0}:null}))).filter(Boolean) as ActiveGame[]
  ...
  async function enter(){
    ...
    try{
      if(mode==='create'){
        ...
        const{data:r,error:e}=await supabase!.from('rooms').select('*').eq('room_code',roomInput.trim().toUpperCase()).neq('status','closed').single();if(e||!r)throw Error('invalid')
        const existing=readSessions().sessions[r.id];if(existing){setSession(existing);setRoom(r);setScreen('lobby');await refresh(existing);return}
        const{data:p,error:pe}=await supabase!.from('players').insert({room_id:r.id,display_name:name.trim(),is_host:false}).select().single();if(pe)throw pe
    ...
    }catch(e){setError(/invalid/.test(String(e))?'That room could not be found or is no longer active.':messageFor(e))}finally{setBusy(false)}
  }
  ...
  async function submit(points:Point[],score:ScoreResult){setResult({points,score});if(!score.valid)return;setScreen('result');if(solo){localStorage.setItem(BEST_KEY,String(Math.max(score.score,Number(localStorage.getItem(BEST_KEY)||0))));return}if(!room||!session)return;await supabase!.from('attempts').insert({room_id:room.id,player_id:session.playerId,round_number:room.current_round,score:score.score,rating:ratingFor(score.score),points:simplify(points),radial_error:score.radialError,closure_error:score.closureRatio,smoothness_score:score.smoothness,angular_coverage:score.angularCoverage});await refresh(session)}
