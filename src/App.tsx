...
export default function App(){
  ...
  const refresh=useCallback(async(s:LocalSession)=>{
    ...
    if(r)setRoom(r);if(p)setPlayers(p);if(a)setAttempts(a);if(rp)setParticipants(rp);setRoomView(restoredRoomView(r.status))
...
function SessionLeaderboard({rows}:{rows:ReturnType<typeof sessionRows>}){return <ul className="stats">{rows.map((row,i)=><li key={row.player.id}><b>{i+1}</b><span><strong>{row.player.display_name}</strong><small>{row.played} played · {row.wins} wins</small></span><em>{row.average.toFixed(1)} avg<br/>{row.best.toFixed(1)} best</em></li>)}</ul>}
