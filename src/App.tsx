...
import { canStartCompetitiveRound } from './roomFlow'
...
export default function App(){
  ...
  const refresh=useCallback(async(s:LocalSession)=>{
    ...
    if(resolved==='submitted_waiting'||resolved==='waiting_next_round'){const{data:status}=await db.rpc('complete_circle_round',{p_room_id:s.roomId,p_round_number:r.current_round});if(status==='results'||status==='finished'){r.status='results';if(status==='finished')r.match_status='finished';resolved='results'}}
  ...
  async function startRound(){if(!room||!session)return;if(!canStartCompetitiveRound(room,players.length,session.isHost)){setError('Waiting for at least one more player');return}const{error:e}=await supabase!.rpc('start_circle_round',{p_room_id:room.id,p_host_player_id:session.playerId});if(e){setError('Could not start the round.');return}setResult(null);setRoomView('game');await refresh(session)}
  ...
  if(screen==='lobby'&&room&&session&&roomView==='results')return <main className="panel room-results"><RoomTabs view={roomView} setView={setRoomView} hasResults={mayRevealCurrent(room)&&currentResults.length>0}/><span className="eyebrow">Round {room.current_round} {room.status==='results'?'results':'leaderboard'}</span><h2>{room.status==='results'?'Final standings':'Current round'}</h2>{mayRevealCurrent(room)?<RoundResults rows={currentResults} myId={session.playerId}/>:<p className="waiting">Scores unlock when everyone finishes.</p>}{room.status==='results'&&(session.isHost?<div className="match-actions"><button className="primary dock" onClick={startRound}>Next round <b>→</b></button><button className="end-game" onClick={endGame}>End Game</button></div>:<p className="waiting">Waiting for host to start the next round</p>)}{updateControl}</main>
