...
describe('async room flow',()=>{
 ...
 it('formats multi-room attention states',()=>{expect(activeGameStatus(room,3,true,1)).toBe('Round 1 · 3/4 players · You submitted');expect(activeGameStatus({...room,expected_player_count:3},3,true,2)).toBe('Round 1 · 3/3 players · Waiting for 1');expect(activeGameStatus(room,3,false)).toBe('Round 1 · Your turn');expect(activeGameStatus({...room,match_status:'finished'},4,true)).toBe('Game complete')})
