...
export function activeGameStatus(room:Pick<Room,'status'|'current_round'|'expected_player_count'|'match_status'|'last_completed_round'>,players:number,submitted:boolean){
  ...
  if(submitted)return`Round ${room.current_round} · ${players}/${room.expected_player_count} players · You submitted`
