...
export function sessionRows(attempts:Attempt[],players:Player[]){
  ...
  return players.map(p=>{const all=attempts.filter(a=>a.player_id===p.id);return{player:p,played:all.length,wins:wins.get(p.id)||0,average:all.length?all.reduce((s,a)=>s+a.score,0)/all.length:0,best:all.length?Math.max(...all.map(a=>a.score)):0}}).sort((a,b)=>b.wins-a.wins||b.average-a.average)
}
