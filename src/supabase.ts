...
export type Room={id:string;room_code:string;host_player_id:string|null;status:'lobby'|'drawing'|'results'|'closed';reveal_mode:'immediate'|'all_submitted';current_round:number;match_status:'active'|'finished';match_length:number;is_extended:boolean;expected_player_count:number;last_completed_round:number}
...
export const messageFor=(e:unknown,fallback='Something went wrong. Please try again.')=>{
  ...
  if(/game_full/i.test(m))return 'This game already has all players.'
